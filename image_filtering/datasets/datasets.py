...
class RedFlashDatasetInvDN(Dataset):
    def __init__(self, data_root, is_cropped = True, is_train = True):
        self.fileID = []
        self.data_root = data_root
        self.is_cropped = is_cropped
...
#             noise_im = utils.validate_im( (noise_im*scale)**0.4 )
        ...
        # run denoiser first
        self.denoiser.feed_test_data(torch.from_numpy(np.expand_dims(np.moveaxis(noise_im, -1, 0), axis=0)).float().to(self.device))
        denoise_im = self.denoiser.test()
